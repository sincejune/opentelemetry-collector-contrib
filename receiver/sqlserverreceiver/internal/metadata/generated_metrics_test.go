...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defaultMetricsCount++
			allMetricsCount++
			mb.RecordSqlserverQuerySample2DataPoint(ts, 1, "str-val")
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "sqlserver.query.sample":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeInt, dp.ValueType())
					assert.Equal(t, int64(1), dp.IntValue())
					attrVal, ok := dp.Attributes().Get("now")
					assert.True(t, ok)
					assert.EqualValues(t, "now-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("query_start")
					assert.True(t, ok)
					assert.EqualValues(t, "query_start-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("user_name")
					assert.True(t, ok)
					assert.EqualValues(t, "user_name-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("last_request_start_time")
					assert.True(t, ok)
					assert.EqualValues(t, "last_request_start_time-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.EqualValues(t, "id-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("last_request_end_time")
					assert.True(t, ok)
					assert.EqualValues(t, "last_request_end_time-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.EqualValues(t, "request_status-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("statement_text")
					assert.True(t, ok)
					assert.EqualValues(t, "statement_text-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("text")
					assert.True(t, ok)
					assert.EqualValues(t, "text-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("client_port")
					assert.True(t, ok)
					assert.EqualValues(t, "client_port-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("client_address")
					assert.True(t, ok)
					assert.EqualValues(t, "client_address-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.EqualValues(t, "program_name-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("is_user_process")
					assert.True(t, ok)
					assert.EqualValues(t, "is_user_process-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.EqualValues(t, "blocking_session_id-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("wait_type")
					assert.True(t, ok)
					assert.EqualValues(t, "wait_type-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("wait_time")
					assert.True(t, ok)
					assert.EqualValues(t, "wait_time-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("last_wait_time")
					assert.True(t, ok)
					assert.EqualValues(t, "last_wait_time-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("wait_resource")
					assert.True(t, ok)
					assert.EqualValues(t, "wait_resource-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("open_transaction_count")
					assert.True(t, ok)
					assert.EqualValues(t, "open_transaction_count-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("transaction_id")
					assert.True(t, ok)
					assert.EqualValues(t, "transaction_id-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("percent_complete")
					assert.True(t, ok)
					assert.EqualValues(t, "percent_complete-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("estimated_completion_time")
					assert.True(t, ok)
					assert.EqualValues(t, "estimated_completion_time-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("cpu_time")
					assert.True(t, ok)
					assert.EqualValues(t, "cpu_time-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("total_elapsed_time")
					assert.True(t, ok)
					assert.EqualValues(t, "total_elapsed_time-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("reads")
					assert.True(t, ok)
					assert.EqualValues(t, "reads-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("writes")
					assert.True(t, ok)
					assert.EqualValues(t, "writes-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("logical_reads")
					assert.True(t, ok)
					assert.EqualValues(t, "logical_reads-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("transaction_isolation_level")
					assert.True(t, ok)
					assert.EqualValues(t, "transaction_isolation_level-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("lock_timeout")
					assert.True(t, ok)
					assert.EqualValues(t, "lock_timeout-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("deadlock_priority")
					assert.True(t, ok)
					assert.EqualValues(t, "deadlock_priority-val", attrVal.Str())
					...
					assert.True(t, ok)
					assert.EqualValues(t, "query_plan_hash-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("context_info")
					assert.True(t, ok)
					assert.EqualValues(t, "context_info-val", attrVal.Str())
				case "sqlserver.query.sample2":
					assert.False(t, validatedMetrics["sqlserver.query.sample2"], "Found a duplicate in the metrics slice: sqlserver.query.sample2")
					validatedMetrics["sqlserver.query.sample2"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "A query sample", ms.At(i).Description())
					assert.Equal(t, "count", ms.At(i).Unit())
