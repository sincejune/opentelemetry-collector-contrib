...
func (m *mySQLScraper) scrapeTopQueries(now pcommon.Timestamp, errs *scrapererror.ScrapeErrors) {
	...
	for i, q := range queries {
		if cached, diff := m.cacheAndDiff(q.digest, q.digest, "timer_wait", q.sumTimerWait); cached && diff > 0 {
...
func (m *mySQLScraper) cacheAndDiff(schemaName string, digest string, column string, val int64) (bool, int64) {
	if val < 0 {
		return false, 0
	}
...
// sortTopQueries sorts the top queries based on the `values` slice in descending order and return the first M(M=maximum) queries
