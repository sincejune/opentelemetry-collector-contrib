...
func (c *postgreSQLClient) getQuerySamples(ctx context.Context, limit int64, logger *zap.Logger) ([]map[string]any, error) {
	...
	for _, row := range rows {
		if row["query"] == "<insufficient privilege>" {
			...
			errs = append(errs, fmt.Errorf("skipping query sample due to insufficient privileges"))
			continue
		}
...
func (c *postgreSQLClient) getTopQuery(ctx context.Context, limit int64, logger *zap.Logger) ([]map[string]any, error) {
	...
	for _, row := range rows {
		...
		needConversion := map[string]func(string, string, *zap.Logger) (any, error){
			"calls":                     convertToInt,
			"max_exec_time":             convertMillisecondToSecond,
			"max_plan_time":             convertMillisecondToSecond,
			"mean_exec_time":            convertMillisecondToSecond,
			"mean_plan_time":            convertMillisecondToSecond,
			"min_exec_time":             convertMillisecondToSecond,
			"min_plan_time":             convertMillisecondToSecond,
			TOTAL_EXEC_TIME_COLUMN_NAME: convertMillisecondToSecond,
			TOTAL_PLAN_TIME_COLUMN_NAME: convertMillisecondToSecond,
			"query": func(column string, val string, logger *zap.Logger) (any, error) {
		...
		for col := range row {
			...
			if hasConvention[col] != "" {
				...
				currentAttributes[DB_ATTRIBUTE_PREFIX+col] = val
			}
		}
		finalAttributes = append(finalAttributes, currentAttributes)
	}
